        ...
        public async Task<IActionResult> GetAgentRequestStatus([FromBody] AgentRequestStatusItem agentRequestStatusItem)
        {
            ...
            switch (workItemDetails.State.ToLowerInvariant())
            {
                ...
                case "unscheduled":
                    return Json(new AgentStatusItem()
                    {
                        statusMessage = $"(Work has been submitted to the Helix API, but has not started yet) Helix work item in job {correlationId} for agent {workItemId} is currently {workItemDetails.State}"
                    });
                case "waiting":
                    return Json(new AgentStatusItem()
                    {
